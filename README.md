...
Scan lab proves the proposed AprilTag shipment-verification loop without seller data or shipment mutations. It can print individual synthetic address labels or a five-label basket sample through the saved label printer, detect the generated preview, a bounded image upload, or an explicitly started webcam, and demonstrate one-match, no-match, two-match, repeated-scan, multiple-visible-tag, and sequential-parcel outcomes. The five basket labels have distinct fake orders and tags in one fake ready-order pool. The 14 mm `tag36h11` marker uses much larger cells than the rejected same-size QR design for fixed-focus webcams. Webcam results require five matching reads for a known tag; an unknown tag needs five exact reads before the lab reports no match, so one noisy frame cannot decide the outcome. The camera remains active between parcels, latches a processed tag while it stays visible, and re-arms after five empty frames. Only the fake in-memory ledger changes. Production order labels do not include fiducial markers, and the lab has no route to mark an order shipped.
...
This release is a single-seller, single-machine service with a CLI, a loopback-only configuration UI, Windows printer discovery, durable fulfillment state, profile-driven one-click exact-SKU inventory additions, searchable inventory with operator-confirmed exact-SKU removal, profile-driven preview-first smart repricing, optional Magic rarity floors, paced mutation queues, and a simulation-only AprilTag shipment-verification lab. Email acceleration, remote administration, production scan-triggered shipment mutations, bulk inventory ingestion, unattended scheduled repricing, per-card pricing floors, and multi-user operation remain deliberate future extensions rather than hidden assumptions in the core workflow.
