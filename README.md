...
Open the printed `http://127.0.0.1:47831` address. The interface opens on persistent, keyboard-accessible Dashboard, Orders, Add cards, Inventory, Settings, and Jobs tabs. Dashboard shows ready-to-ship orders and compact dry-run, address-label, and packing-slip switches that mirror the full controls in Settings. The Orders tab shows the last three months of orders and provides explicit controls to print or download fulfillment documents, add tracking, mark an order shipped, or open it in the TCGplayer Seller Portal. Order data remains in memory and is briefly cached to avoid repeated Seller Portal requests; it is not written to application state or logs.

Polling, printer details, merchandise profiles, and background-worker configuration remain in Settings. Dashboard switches use the same save banner and do not replace those full controls. Real-order printing, tracking, and shipment changes are rejected while dry run is enabled. The UI discovers installed Windows printers and provides independent on/off controls for address labels and packing slips, along with label-size, font, PDF scaling, DPI, and queue settings. Add Cards uses a compact merchandise-profile selector above the catalog search. Each result shows its regular and English Near Mint Foil market prices, a condition selector, a Foil toggle, and `+1`, `+2`, `+3`, `+4`, and `+X` quantity controls. Choosing a quantity expands only that row with language, live preview, and the queue action. Product details remain lazy-loaded for only the selected row. Catalog search is ranked by name likeness and includes product images. The UI listens only on this computer and never receives or displays the seller credentials from `.env.local`.
...
2. In **Add cards**, select the profile for this batch, then search for the card name and optionally narrow by product line. The application requests one TCGplayer page first and expands to at most three pages only while exact or qualified name matches remain scarce. A normalized likeness score keeps exact names, word-order variants, partial names, and typo-tolerant fuzzy matches correctly ordered across every loaded page. `Load more` requests one additional page; when no exact result is loaded, `Find exact name` explicitly scans up to eight later pages and stops as soon as it finds one. Identical search batches are cached briefly, and replaced searches are canceled.
3. On the exact set row, select condition, toggle **Foil** when needed, and choose `+1`, `+2`, `+3`, or `+4`. Use `+X` for any other quantity. Regular and English Near Mint Foil market prices appear beside the set metadata; a dash means no matching live Foil SKU was available for price enrichment. The selected row expands without hiding the other search results and offers the languages available for that exact condition and printing. Unsupported Foil, condition, and language combinations stop at preview instead of silently switching SKU.
4. Review the live preview, which updates automatically after changes and coalesces rapid edits into one request at a time. Selecting a SKU loads its live seller inventory and comparison listings into a 15-minute in-memory snapshot; quantity and profile-policy changes recalculate from that snapshot without repeating TCGplayer requests. For a standalone item under $5, delivered-price comparison applies TCGplayer's current $1.49 minimum shipping charge. Verify the seller quantity and proposed initial price, then select **Add to queue**.
