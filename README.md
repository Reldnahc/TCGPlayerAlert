...
Scan lab proves the proposed QR shipment-verification loop without seller data or shipment mutations. It can print one of three synthetic address labels through the saved label printer, decode the generated preview, a bounded image upload, or an explicitly started webcam, and demonstrate one-match, no-match, two-match, and repeated-scan outcomes. Only the fake in-memory ledger changes. Production order labels do not include QR codes, and the lab has no route to mark an order shipped.
