...
Ranges are chosen by the lowest qualifying listing, falling back to market price only when no comparable exists. This prevents a product-level market figure from placing a particular condition or printing in the wrong risk tier. Comparable searches merge ordinary marketplace channel-0 listings with verified channel-1 Direct offers, then apply exact product, printing, language, and condition rules. A channel-1 record is eligible only when TCGplayer simultaneously reports a Direct listing, Direct-capable product, Direct seller, positive Authentication Center inventory, the `standard` listing type, and the `DirectViewable` seller program. Missing evidence fails closed. Seller-band analysis first keeps only each seller's cheapest eligible listing across both channels, so one seller cannot create artificial support with several listings. Starting at the absolute low, it finds the cheapest price whose configured percentage window contains the required number of distinct sellers. The isolated-low gap compares the absolute low with that supported band's starting price. A low already supported by enough sellers remains the reference; an unsupported low can be ignored only when a supported band exists above it. When minimum comparables, seller-band support, or the selected price source is unavailable, the profile may wait, use the higher of market and lowest, prefer market then lowest, or prefer lowest then market. If the selected fallback has neither source, the item remains unqueued with a warning rather than a listing failure.

Marketplace inventory and comparison data are cached in memory for ten minutes. For each unique product, printing, language, and allowed-condition set, the repricer requests the real price-sorted product-listing page with marketplace and Direct channels combined. It loads one page of up to 50 rows first and loads at most two more pages only when the selected profile still lacks its required listing or seller support. Requests remain sequential, simultaneous previews share in-flight work, and repeated previews reuse the snapshot. This is normally one comparison request per unique product on a fresh snapshot; it deliberately trades more bounded read traffic for correct listing data. The repricer never prices from TCGplayer's three-row product spotlight sample, and the UI counts only qualifying rows actually loaded rather than presenting TCGplayer's product total as inspected comparables. For delivered-price profiles, ordinary marketplace listings below $5 use a normalized $1.49 shipping component for sorting, seller-band support, gap detection, and target calculation; actual returned shipping remains visible. Verified Direct listings retain their returned shipping. **Force marketplace refresh** deliberately reloads the snapshot. The cache is process-local, expires automatically, and contains no credentials.
