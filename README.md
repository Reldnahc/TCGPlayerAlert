...
The Dashboard can also print a pasted multiline address through the saved address-label printer and layout. The separate **Labels** workspace accepts up to 100 unrelated addresses as blank-separated blocks, one-line addresses, CSV, or tab-separated spreadsheet rows; it formats them into editable previews and prints one label at a time. A failure stops the batch and keeps submitted, failed, and remaining labels distinct so nothing is retried automatically. Both explicit manual actions work independently of the automation enable switch and do not add pasted addresses to durable application state or logs.

Scanner is the opt-in production AprilTag workflow. When shipment scanning is enabled, configured order-address labels receive a stable 14 mm `tag36h11` marker; the marker contains no order or customer data. The application durably reserves marker IDs for the current ready-order set, starts each order at its versioned hash-derived candidate, and moves collisions to an unused ID before printing. Reprints keep the same reservation across restarts, concurrent reservations are serialized, and printing stops before submission if no unique ID is available. Reservations are stored beside the mutation ledger in `.data/shipment-scans.json.tags` and reconciled only from an authoritative ready-order response. When a confirmed response no longer includes an order, its marker becomes retired and remains unavailable until 100 newer order markers have been assigned; elapsed wall time alone never releases it. A separately enabled background-camera adapter belongs to the long-running app process, so closing the operator console does not stop capture or detection. Select the camera under **Settings → Scanning** and keep `npm start` running. The Scanner page shows a once-per-second grayscale preview copied from that backend stream; it never opens the camera itself. The preview is downsampled, served only through the no-cache loopback UI, overwritten in memory, and never saved or logged. After five matching camera reads, the server performs one fresh authoritative ready-to-ship search and requires exactly one reserved matching order. Review mode pauses for an explicit shipment action; automatic mode marks the exact match shipped immediately without an open page. Zero matches, repeated successful scans, interrupted mutations, unsafe assignment state, and uncertain seller responses never guess or retry. Mutation intent and outcomes are kept in `.data/shipment-scans.json` without buyer or address data. Scanner status, camera discovery, preview reads, and ordinary frames do not issue seller requests. Optional host-system success and failure sounds supplement the visual result.
...
TCGplayer Seller Portal links open in a new tab. Navigation within the local
operator app stays in the current tab.
