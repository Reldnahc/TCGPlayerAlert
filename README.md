...
4. Review the live preview, which updates automatically after changes and coalesces rapid edits into one request at a time. Selecting a SKU loads its live seller inventory and comparison listings into a 15-minute in-memory snapshot; quantity, minimum, shipping, and adjustment changes recalculate from that snapshot without repeating TCGplayer requests. Changing the condition-comparison policy loads that comparison set once. Verify the seller quantity and proposed initial price, then select **Add to queue**.
