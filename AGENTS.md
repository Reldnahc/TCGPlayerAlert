...
- **Order presentation:** seller-confirmed, in-memory order details and a consolidated ready-order master pull list behind the loopback UI boundary, with explicit fulfillment controls, optional provider-supplied card metadata, capability-gated full and partial refunds, and an official-portal escape hatch; do not persist or log addresses, products, pull sheets, refund messages, or raw provider actions.
...
This application repository owns scheduling, manual synchronization, reconciliation, optional event accelerators, domain orchestration, rules, actions, printing, user configuration, read-only payment and feedback presentation, message presentation and operator controls, the mandatory order-refund confirmation experience, and end-to-end workflow state. The API repository owns TCGplayer seller authentication/session behavior, private endpoint transport, response validation, order and document retrieval, order-refund option and mutation transport, read-only payout and feedback retrieval, and seller-message reads, mark-read, and reply transport.
...
- Never automatically retry an order refund. An ambiguous refund result must block resubmission until the operator reconciles the order with TCGplayer.
