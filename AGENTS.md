...
- **Messages:** cached, read-only seller inbox, unread count, and thread content normalized behind an application service; keep content in memory, do not log it, and leave replies and read-state management in TCGplayer.
...
This application repository owns scheduling, manual synchronization, reconciliation, optional event accelerators, domain orchestration, rules, actions, printing, user configuration, read-only payment, feedback, and message presentation, and end-to-end workflow state. The API repository owns TCGplayer seller authentication/session behavior, private endpoint transport, response validation, order and document retrieval, and read-only payout, feedback, and message retrieval.
