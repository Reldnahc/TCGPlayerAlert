...
This application repository owns scheduling, manual synchronization, reconciliation, optional event accelerators, domain orchestration, rules, actions, printing, user configuration, read-only payment and feedback presentation, and end-to-end workflow state. The API repository owns TCGplayer seller authentication/session behavior, private endpoint transport, response validation, order and document retrieval, and read-only payout and feedback retrieval.
