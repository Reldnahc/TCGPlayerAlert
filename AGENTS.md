...
- **Messages:** cached seller inbox, unread count, and thread content normalized behind an application service, with explicit mark-read and reply actions; keep content and drafts in memory, never log them, and stop uncertain replies until the operator refreshes the thread.
...
This application repository owns scheduling, manual synchronization, reconciliation, optional event accelerators, domain orchestration, rules, actions, printing, user configuration, read-only payment and feedback presentation, message presentation and operator controls, and end-to-end workflow state. The API repository owns TCGplayer seller authentication/session behavior, private endpoint transport, response validation, order and document retrieval, read-only payout and feedback retrieval, and seller-message reads, mark-read, and reply transport.
