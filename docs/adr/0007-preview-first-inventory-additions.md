...
Catalog results expose a Foil toggle followed by fixed `+1` through `+4` quantity controls and a custom-quantity dialog. The selected row expands in place while the remaining results and search controls stay visible. Foil is an exact printing choice and receives a visible row border; the application never silently falls back to Normal when a Foil SKU is unavailable. The expanded row offers only the conditions and languages valid for that printing.
