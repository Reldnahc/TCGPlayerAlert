...
Catalog discovery is explicitly paginated because TCGplayer's fuzzy search can return thousands of products before local likeness ranking. The application scans a bounded three-page batch, ranks that complete batch, separates exact names from qualified variants and related results, exposes truthful total/scanned counts, and lets the operator load subsequent batches. Product-line suggestions come only from validated products already returned by TCGplayer; an operator may still enter an exact product-line name when it has not appeared yet.
