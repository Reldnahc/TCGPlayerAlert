...
- Organize the growing operator surface into Settings, Add cards, Repricing, and Jobs tabs. Keep persistent polling, dry-run, printing, and background-worker configuration in Settings; keep action inputs with their workflows; and reserve Jobs for queue history and controls on individual jobs. Preserve each tab's DOM state while navigating, expose the selected tab in the URL hash, remember it locally, and support standard arrow, Home, and End keyboard navigation. Treat the former `automation` tab hash or stored selection as `settings` for compatibility.
