...
- Organize the growing operator surface into Dashboard, Orders, Add cards, Inventory, Settings, and Jobs tabs. Dashboard is the default and shows only ready-to-ship order summaries plus compact enable/disable mirrors for dry run, address-label automation, and packing-slip automation. Orders shows the last three months of order summaries and explicit per-order fulfillment actions. Keep persistent polling, detailed printing, and background-worker configuration in Settings; keep action inputs with their workflows; and reserve Jobs for queue history and controls on individual jobs. Preserve each tab's DOM state while navigating, expose the selected tab in the URL hash, remember it locally, and support standard arrow, Home, and End keyboard navigation. Treat the former `automation` tab hash or stored selection as `settings`, and the former `repricing` selection as `inventory`, for compatibility.
